...
class Day2 {
    ...
    fun one(input: String): Int {
        ...
        var counter = 0
        val maxColor = HashMap<String, Int>();

        maxColor["red"] = 12
        maxColor["green"] = 13
        maxColor["blue"] = 14

        val a = lines.map { checkGame(it, maxColor) }
            .sum()
        return a;
    }

    fun checkGame(line: String, maxColors: HashMap<String, Int>): Int {
        val game = line.substringBefore(":").substringAfter(" ")
        val a=line.substringAfter(": ")
            .split("; ")
            .map { it.split(", ") }
            .map { b -> processSet(b,maxColors) }
            .all{it}
        return if (a) game.toInt() else 0
    }

    fun processSet(cubes: List<String>, maxColors: HashMap<String, Int>): Boolean {
        for(set in cubes){
            val items  = set.split(" ");
            val number :Int = items[0].toInt()
            val color : String = items[1]
            val maxColor=maxColors[color]
            if(maxColor!! < number){
                return false
            }
        }
    ...
    fun two(input: String): Int {
        ...
        val a = lines.map { checkGame(it) }
            .sum()

        return a;
    }

    fun checkGame(line: String): Int {
        val game = line.substringBefore(":").substringAfter(" ")
