...
class Day3 {
    ...
    fun one(input: String): Int {
        ...
        matrix.onEachIndexed { row, item ->
            numbers += processLine(item, row, matrix)
        }

        return numbers.sum()
    }

    fun processLine(line: CharArray, row: Int, matrix: Array<CharArray>): List<Int> {
        var start = -1
        var end = -1
        val numbers: MutableList<Int> = mutableListOf()
        line.forEachIndexed { col, char ->
            if (char.isDigit()) {
                ...
                if (start != -1) {
                    if (hasSymbolNeighbour(start, col, row, matrix)) {
        ...
        if (start != -1) {
            ...
            if (hasSymbolNeighbour(start, endOfLine, row, matrix)) {
    ...
    fun hasSymbolNeighbour(startCol: Int, endCol: Int, row: Int, matrix: Array<CharArray>): Boolean {
        ...
        if (row > 0) {
            ...
            for (cur in startCol..<endCol) {
                neighbours += matrix[row - 1][cur]
            }

            if (endCol <= matrix.size - 1) {
                neighbours += matrix[row - 1][endCol]
            }
        }
