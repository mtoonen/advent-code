...
public class Second {
    ...
    public int checksumMinMax(String[] input) {
        int total = 0;
        for (String row : input) {
            total += differenceRow(row);
        }
        return total;
    }

    public int differenceRow(String r) {
    ...
    public int checkSumNoRemainder(String[]input){
         int total = 0;
        for (String row : input) {
            total += differenceRowNoRemainder(row);
        }
        return total;
    }
    
    public int differenceRowNoRemainder(String r) {
        int min = Integer.MAX_VALUE;
        int max = 0;
        String[] chars = r.split(" ");
...
"114 605 94 136 96 167 553 395 164 159 284 104 530 551 544 18"};
        Second s = new Second();
        System.out.println(s.checkSumNoRemainder(input));
    }
